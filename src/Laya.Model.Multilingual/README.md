...
The multilingual checkpoint for [Laya](https://github.com/theolivenbaum/laya) — the non-autoregressive System 1
decision engine for .NET. Covers 100+ languages.

```csharp
using Laya;
using Laya.Runtime;

// First call downloads the weights (644 MB) into ~/.cache/laya; later calls reuse them.
...
| Parameters | 322M |
| Max tokens | 1024 |
| Weights | 644 MB, downloaded from <https://models.curiosity.ai/laya/multilingual/model.safetensors> |
...
`LayaMultilingual.Prepare()` during deployment to warm the cache ahead of first use.
